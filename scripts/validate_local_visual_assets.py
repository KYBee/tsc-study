#!/usr/bin/env python3
"""Validate tracked working images and their development-only serving boundary."""
...
ASSET_ROOT = ROOT / "data/working/app-assets/tsc-individual-images-v1"
DIST_DIR = ROOT / "dist"
FIXTURE_DIRS = {
...
def validate_part(part: int) -> None:
    ...
    if len(records) != expected_count:
        fail(
            f"Expected {expected_count} Part {part} VisualAssets, "
            f"found {len(records)}"
        )
    ...
    for record in records:
        ...
        if ignored.returncode == 0:
            fail(f"Working app asset is unexpectedly ignored by Git: {path_text}")
    ...
    print(
        ...
        f"{len(seen_hashes)} unique SHA-256 values, Git-ready, "
