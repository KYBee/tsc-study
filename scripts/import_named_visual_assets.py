...
DEFAULT_ARCHIVE = ROOT / "data" / "raw" / "TSC_individual_images_named.zip"
DEFAULT_OUTPUT = (
    ...
    output["manifest.json"] = json_bytes(
        {
            ...
            "source_archive": repository_path(archive),
...
def validate_output(output_dir: Path, archive: Path) -> dict[str, int]:
    if not output_dir.is_dir():
        raise ImportError(f"asset output is missing: {output_dir}")
    expected = build_files(archive)
    ...
    for name, value in expected.items():
        if (output_dir / name).read_bytes() != value:
            raise ImportError(f"asset output differs from source archive: {name}")
...
def publish(output_dir: Path, archive: Path) -> dict[str, int]:
    ...
    try:
        ...
        validate_output(staging, archive)
        ...
        return validate_output(output_dir, archive)
    except Exception:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        counts = (
            validate_output(args.output_dir, args.archive)
            if args.validate_only
            else publish(args.output_dir, args.archive)
        )
