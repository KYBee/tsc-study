...
| 기본 화면에서 답변 저장 | `question`, `visual_question`, `visual_set` 대상별 활성 `PracticeDraft`를 각각 upsert한다. 답변 저장은 `ReviewState`를 만들거나 변경하지 않음 |
...
| 기본 암기 버튼 선택 | `못 외움` 또는 `외움`을 해당 대상의 `ReviewState`로 명시적으로 upsert. 기존 `헷갈림` 값은 사용자가 다른 값을 누르기 전까지 보존 |
...
| 주요 행동 | 기본 필터(전체·미작성·작성 완료·못 외움·외움), 문제 상세의 즉시 초안 저장, 못 외움·외움 저장, 이전·다음 이동. 검색·유형·상세 상태 필터와 구조화 작성·교정·회상은 보조 행동으로 유지 |
...
| 주요 행동 | 세트 선택 후 네 VisualQuestion마다 별도 자유 입력 저장과 못 외움·외움 저장. 이미지 확대·상세 질문 이동·실전 모드·추천 답변 비교·상세 회상은 보조 행동으로 유지 |
...
| 주요 행동 | 네 장면을 보며 VisualSet 대상 이야기 답변을 즉시 저장하고 못 외움·외움을 별도 저장. StoryGuide·구조화 포인트 편집·상세 회상·연결 상태는 보조 행동으로 유지 |
