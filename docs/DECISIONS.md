...
Phase 0에서 요구사항과 데이터 운영 원칙을 정리했으며, 다음 단계는 아직 시작 전인 Phase 1 자료 적재 및 표본 검수다. 아래 항목은 자료 표본과 실제 구현 요구를 검토하기 전까지 **미결정**으로 유지한다. 편의를 위해 임시 선택을 하더라도 프로젝트의 확정 결정으로 기록하지 않는다.
...
| `D-007` | Excel, CSV, JSON 중 기준 데이터 형식 | 미결정 | 실제 Excel 구조, 사람이 검수하는 방식, 변환 손실, 변경 이력 관리 |
