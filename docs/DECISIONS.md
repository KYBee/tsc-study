...
| `D-027` | 로컬 이미지 권리 경계(초기) | 초기 workbook 추출 바이트는 Git ignore 생성 경로에 두고 Vite `serve` 전용 allowlist 미들웨어로만 제공했다. production 제외와 권리 경계는 유지하며, Git 저장 정책은 이후 `D-032`로 갱신했다. | VisualAsset별 공개 권리 근거가 승인될 때 |
...
| `D-029` | Part 7 VisualSet 중심 working fixture | `part7-visual-working-development-fixture-v1`의 VisualSet·StoryGuide·Question 각 12개와 현재 VisualAsset·VisualSetAsset 각 48개를 병행 로드한다. 확정 QuestionVisualSet과 ModelAnswer는 0개다. 숫자 접미사 후보 12개는 `candidate`, `review_needed`, `not_canonical`로만 보존하고 VisualSet을 직접 학습 대상으로 쓴다. | 후보 관계가 사람 검수로 승인되거나 reviewed Part 7 데이터가 준비될 때 |
| `D-030` | Part 2·7 공용 로컬 이미지 경계(초기) | 초기 workbook asset ID 24개를 Vite `serve` allowlist로 제공했다. realpath root, 확장자·MIME magic, 크기·SHA 검증과 production 비활성 원칙은 유지하며 자산 수·Git 정책은 `D-032`로 갱신했다. | VisualAsset별 공개 권리 근거와 production 자산 정책이 승인될 때 |
...
| `D-032` | 사용자 제공 이름 지정 시각 자산 | 원본 ZIP은 `data/raw`에 바이트 그대로 보존하고, 안전한 deterministic importer가 Part 2 12장·Part 7 48장을 `data/working/app-assets/tsc-individual-images-v1/`에 재인코딩 없이 푼다. 명시적 파일명·CSV의 Part 7 장면 1~4 순서를 사용하며 60개 ID만 개발 서버 allowlist로 제공한다. 사용자 요청에 따라 working PNG를 Git에 보존하지만 `rights_status = review_needed`, `public_allowed = false`, production 화면·build 제외는 유지한다. | 이미지 원출처·공개 권리가 사람 검수로 승인되거나 production 자산 정책을 정할 때 |
