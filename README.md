...
첫 원본 Excel을 원래 내용과 파일명 그대로 `data/raw`에 적재하고, 시트 구조와 향후 매핑을 읽기 전용으로 조사했다. 아직 전체 문제 데이터 추출·검수와 모범답안 작성은 시작하지 않았으며 웹 앱 코드, 패키지, 데이터베이스, API 연결도 없다. 기술 스택과 기준 데이터 형식은 여전히 미결정이다.
...
1. [원본 Excel 구조 조사](docs/WORKBOOK_INVENTORY.md)와 [향후 매핑](docs/WORKBOOK_MAPPING.md)을 검토한다.
2. 원본의 출처와 링크·이미지 공개 가능 여부를 별도로 확인한다.
3. 일부 대표 자료로 추출·중복 확인·정규화·검수 흐름을 시험한다.
