...
- **문제 복습:** 답변을 숨긴 문제 풀이와 `못 외움`·`헷갈림`·`외움` 상태 관리
...
현재는 **Phase 0: 프로젝트 지침과 문서** 단계다. 웹 앱 코드, 패키지, 데이터베이스, API 연결은 아직 없다. 데이터 형식과 기술 스택도 결정하지 않았다.
...
1. 현재 문서의 요구사항과 미결정 사항을 검토한다.
2. 사용자가 제공하는 강의 분석 자료와 예상 문제 Excel의 출처와 공개 가능 여부를 확인한다.
3. 원본을 `data/raw`와 `sources` 운영 원칙에 맞게 보관한다.
4. 일부 자료로 추출·중복 확인·정규화·검수 흐름을 시험한다.
5. 검증 결과를 바탕으로 기준 데이터 형식과 구현 기술을 별도로 결정한다.
