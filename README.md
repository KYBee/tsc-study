...
첫 원본 Excel을 원래 내용과 파일명 그대로 `data/raw`에 적재하고 시트 구조를 조사한 뒤, Part 1~4의 대표 Question 20개를 `data/working/question-sample`에 작업용 CSV로 추출했다. 아직 전체 253개 문제 변환·검수와 모범답안 작성은 시작하지 않았으며 웹 앱 코드, 패키지, 데이터베이스, API 연결도 없다. 기술 스택과 기준 데이터 형식은 여전히 미결정이다.
...
1. [Question 대표 표본 보고서](docs/SAMPLE_IMPORT_REPORT.md)의 스키마 검토 필요 사항을 확인한다.
2. 원본의 출처와 링크·이미지 공개 가능 여부를 별도로 확인한다.
3. Part 5~7과 그림 자료의 추가 표본으로 매핑을 검증한다.
