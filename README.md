...
첫 원본 Excel을 원래 내용과 파일명 그대로 `data/raw`에 적재하고 시트 구조를 조사한 뒤, Part 1~4의 대표 Question 20개와 Part 5~7의 대표 Question 14개를 작업용 CSV로 추출했다. Part 2·7의 그림 세트, 원본 추천 답변과 StoryGuide도 각각 2세트씩 표본 검증했다. 아직 전체 253개 문제와 전체 시각 자료의 변환·검수, 새 모범답안 작성은 시작하지 않았으며 웹 앱 코드, 패키지, 데이터베이스, API 연결도 없다. 기술 스택, 기준 데이터 형식과 시각 자료 엔터티 구조는 여전히 미결정이다.
...
1. [Question 대표 표본 보고서](docs/SAMPLE_IMPORT_REPORT.md)와 [확장 표본 보고서](docs/EXTENDED_SAMPLE_REPORT.md)의 검토 필요 사항을 확인한다.
2. 시각 자료 엔터티, 연결되지 않은 시각 질문과 `answer_point`의 구조를 결정한다.
3. 원본의 출처와 링크·이미지 공개 가능 여부를 별도로 확인한다.
4. 전체 추출 범위를 정한 뒤 기준 데이터 형식과 구현 기술을 별도로 결정한다.
